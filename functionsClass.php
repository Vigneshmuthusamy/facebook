...
class functionsClass {
    ...
    public function insertImage($statusMsg,$url,$scheduledTime){
        
        $sql = "INSERT INTO `share` (`id`, `status_message`, `photo_link`, `date_time`, `is_status`, `is_queued`) VALUES (NULL, '{$statusMsg}','{$url}', '{$scheduledTime}', '0', '1');";
    ...
    public function insertStatusLinks($statusMsg, $statusLink, $statusImageLink, $scheduledTime){
         $sql = "INSERT INTO `share` (`id`, `status_message`, `status_link`, `status_image_link`, `date_time`, `is_status`, `is_queued`) VALUES (NULL, '{$statusMsg}','{$statusLink}', '{$statusImageLink}','{$scheduledTime}', '1', '1');";
         if($results = mysql_query($sql)){            
            $status = 'success';
        }else{            
            $status ="error";
    ...
    public function queuedRequest(){
        $sql = "SELECT * FROM `share` where `is_queued` = 1 ORDER BY `date_time` asc";
    ...
    public function archievedRequest(){
         $sql = "SELECT * FROM `share` where `is_queued` = 0 ORDER BY `date_time` desc LIMIT 5";
        $results = mysql_query($sql);
	$data=array();
	if(is_resource($results)){
            $count = mysql_num_rows($results);
	...
	public function setAccessToken($accessToken){
        $sql = "update accesstoken set accesstok='".$access_token."' where sno='1'";
        mysql_query($sql);
        $affRows = mysql_affected_rows();
        echo "Records affected: " . $affRows;
